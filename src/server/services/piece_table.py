...
class PieceTable:
    ...
    #TODO split up parts into separate functions    
    def open_block(self, start: int, length: int) -> int:
        ...
        for block_index in range(range_start, range_end + 1):
            if self.blocks[self.table[block_index][0]].status() == True:
        ...
        if rem > 0:
            ...
            self.table.insert(index + 2, [self.table[index][0], 
                                          self.table[index][1] + offset + length, 
                                          rem])
        else:
