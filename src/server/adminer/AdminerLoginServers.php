 ...
 * Configuration is similar to the original "login-servers" plugin but â€“ the killer feature â€“ each server can have
 * a different driver!
 *
...
class AdminerLoginServers
{
    ...
    public function __construct(array $servers, $defaultDriver = "mysql")
    {
        ...
        $this->parseServers($servers, $this->servers,
            $this->drivers, $this->databases);
    }
    ...
    private function parseServers(array $servers, array &$out,
        array &$drivers, array &$databases)
    {
        foreach ($servers as $key => $value) {
            if (is_array($value)) {
                ...
                list($driver, $server, $database) = $this->parseServer(
    ...
    private function parseServer($server)
    {
        ...
        $driver = $matches[2];
        $server = $matches[3];
        $database = count($matches) >= 6 ? $matches[5] : null;
            ...
            (function(document) {
                ...
                var servers = {
                    ...
                    foreach ($this->drivers as $server => $driver) {
                        $db = $this->databases[$server];
                        if ($db)
                            $db = "'$db'";
                        else
                            $db = "null";

                        echo "'$server': ['$driver', $db]";
                        if ($i++ < $count) {
                            echo ", ";
                        }
                    }
